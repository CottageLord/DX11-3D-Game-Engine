...
namespace GPipeline
{
	...
	*/
	...
	Surface GPipeline::DepthStencil::ToSurface(Graphics& gfx, bool linearlize) const
	{
		GET_INFO_MAN(gfx);
		namespace wrl = Microsoft::WRL;
	...
	unsigned int GPipeline::DepthStencil::GetWidth() const
	{
		return width;
	}

	unsigned int GPipeline::DepthStencil::GetHeight() const
	{
		return height;
	}
