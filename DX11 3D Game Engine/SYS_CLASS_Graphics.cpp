	...
	height(height)
{
	...
	pTarget = std::shared_ptr<GPipeline::RenderTarget>{ new GPipeline::OutputOnlyRenderTarget(*this,pBackBuffer.Get()) };
	...
	dsDesc.DepthFunc = D3D11_COMPARISON_LESS; // z less -> closer to screnn -> chosen
	...
	*/
	...
	descDepth.MipLevels = 1u;					// mipmap level
	descDepth.ArraySize = 1u;
	descDepth.Format = DXGI_FORMAT_D24_UNORM_S8_UINT;	// D32 specially for depth 
												// could also use 16 norm to split between stencil & depth
	descDepth.SampleDesc.Count = 1u;			// anti-aliasing stuff
...
std::shared_ptr<GPipeline::RenderTarget> Graphics::GetTarget()
{
	return pTarget;
}
