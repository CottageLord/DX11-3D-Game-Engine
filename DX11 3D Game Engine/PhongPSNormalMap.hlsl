/// a phong ps shader using tangent space normal map
cbuffer LightCBuf
{
    float3 lightPos;
    float3 ambient;
...
Texture2D tex;
Texture2D nmap : register(t2); // save t1 for possible specular data

SamplerState splr;

// take in the tagents for interpolation
float4 main(float3 viewPos : Position, float3 viewNormal : Normal, float3 tan : Tangent, float3 bitan : Bitangent, float2 tc : Texcoord) : SV_Target
{
    ...
    if (normalMapEnabled)
    {
        ...
        tanNormal.y = -tanNormal.y; // openGL to DX translate
