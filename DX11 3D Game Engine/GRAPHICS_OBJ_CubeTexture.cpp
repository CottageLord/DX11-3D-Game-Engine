...
namespace GPipeline
{
		...
		slot(slot)
	{
		...
		// dist doesn't matter, therefore mip level always 1
		...
		textureDesc.MiscFlags = D3D11_RESOURCE_MISC_TEXTURECUBE; // box texture
		// subresource data
		...
		for (int i = 0; i < 6; i++)
		{
			...
			data[i].SysMemSlicePitch = 0; // not useful here
		}
		...
		srvDesc.ViewDimension = D3D11_SRV_DIMENSION_TEXTURECUBE; // box texture
	...
	std::shared_ptr<OutputOnlyRenderTarget> GPipeline::CubeTargetTexture::GetRenderTarget(size_t index) const
	{
	...
	std::shared_ptr<OutputOnlyDepthStencil> GPipeline::DepthCubeTexture::GetDepthBuffer(size_t index) const
	{
