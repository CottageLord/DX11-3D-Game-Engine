	...
	height(height)
{
	...
	hWnd = CreateWindow(
		...
		nullptr, nullptr, WindowClass::GetInstance(), this // <- previously null, used to establish future access
															// when we are in the message handling
	);
	...
	// show window, newly created window starts off ashidden
	...
	// this enables accessing windows raw data
	RAWINPUTDEVICE rid;
	rid.usUsagePage = 0x01; // mouse page
	rid.usUsage = 0x02; // mouse usage
	rid.dwFlags = 0;
	rid.hwndTarget = nullptr; // don't need to specify here
...
Graphics& Window::Gfx()
{
	if (!pGfx) throw MFWND_NOGFX_EXCEPT();
	return *pGfx;
}
...
LRESULT Window::HandleMsg(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) noexcept
{
	...
	case WM_KILLFOCUS:
		...
		break;
	// handle mouse event when switching between windows
	case WM_ACTIVATE:
	...
	case WM_MOUSEMOVE:
	{
		...
		{
			// if we drags outside the region
			// could also use LeftIsPressed()
	...
	case WM_INPUT:
	{
		if (!mouse.RawEnabled()) { break; }
		UINT size = 0;
		// first get the size of the input data
		if (GetRawInputData(
			reinterpret_cast<HRAWINPUT>(lParam),
			RID_INPUT,
			nullptr, // call with nullptr to get data size
			&size,
		...
		// initialize the buffer to the retrived size
	...
	case WM_MOUSEWHEEL:
	{
		...
		if (imio.WantCaptureMouse) { break; }
		const POINTS pt = MAKEPOINTS(lParam);
		// wParam: delta value determines scroll direction
...
{
	return "Main Window Exception [No Graphics]";
}
