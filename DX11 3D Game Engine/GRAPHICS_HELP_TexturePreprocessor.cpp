...
template<typename F>
inline void TexturePreprocessor::TransformSurface(GPipeline::Surface& surf, F&& func)
{
...
{
	auto surf = GPipeline::Surface::FromFile(pathIn);
...
void TexturePreprocessor::MakeStripes(const std::string& pathOut, int size, int stripeWidth)
{
	...
	GPipeline::Surface s(size, size);
	for (int y = 0; y < size; y++)
	{
		for (int x = 0; x < size; x++)
		{
			GPipeline::Surface::Color c = { 0,0,0 };
...
DirectX::XMVECTOR TexturePreprocessor::ColorToVector(GPipeline::Surface::Color c) noexcept
{
	using namespace DirectX;
...
GPipeline::Surface::Color TexturePreprocessor::VectorToColor(DirectX::FXMVECTOR n) noexcept
{
	using namespace DirectX;
	const auto all1 = XMVectorReplicate(1.0f);
