...
public:
	...
	void SetKernelGauss(Graphics& gfx, int radius, float sigma) noxnd
	{
		...
		for (int i = 0; i < k.nTaps; i++)
		{
			k.coefficients[i].x /= sum; // makes sure the total sum is 1
		}
		pcb.Update(gfx, k);
	}
	/**
	 * @brief Set the blur effect to box filter - all values in the matrix is same
	 */
...
private:
	...
	struct Kernel
	{
		int nTaps; // how much space are we going to use in the coefficients[]
		float padding[3];
	...
	GPipeline::PixelShader shader;
	GPipeline::PixelConstantBuffer<Kernel> pcb;
	GPipeline::PixelConstantBuffer<Control> ccb;
};