...
namespace Rgph
{
	RenderGraph::RenderGraph(Graphics& gfx)
		...
		masterDepth(std::make_shared<GPipeline::OutputOnlyDepthStencil>(gfx))
	{
		// setup global sinks and sources
		AddGlobalSource(DirectBufferSource<GPipeline::RenderTarget>::Make("backbuffer", backBufferTarget));
		AddGlobalSource(DirectBufferSource<GPipeline::DepthStencil>::Make("masterDepth", masterDepth));
		AddGlobalSink(DirectBufferSink<GPipeline::RenderTarget>::Make("backbuffer", backBufferTarget));
	}
